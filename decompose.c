...
int iunit(mpz_t iu, const mpz_t p)
{
  ...
  if (mpz_tstbit(p, 2)) {
    // with pre-conditions, this implies q & 7 == 5 (q == 0b...101)
    flag = 1;
    mpz_set_ui(q, 2);
  } else {
    flag = 0;
...
{
  // pre-condition: n is a prime, n % 4 == 1 (p == 0b...01 )
...
int decompose(mpz_t fours[FOUR], const mpz_t n)
{
  ...
  mpz_t num, r0, r1, v, sqr, p, delta;
  mpz_inits(num, r0, r1, v, sqr, p, delta, NULL);
  mpz_set(num, n);

  mpz_set_ui(v, 1);
  while(!mpz_tstbit(num, 0) && !mpz_tstbit(num, 1)) { // num & 3 == 0
    mpz_tdiv_q_2exp(num, num, 2);
    mpz_add(v, v, v);
  }

  // post-condition:
  // 1. original num = v^2 * num
  // 2. num % 4 != 0
  ...
  if (!mpz_sgn(p)) {
    ...
    mpz_clears(num, r0, r1, v, sqr, p, delta, NULL);
    return 1;
  }

  if (mpz_tstbit(num, 0) && !mpz_tstbit(num, 1)) { // n & 3 == 1
    if (mpz_probab_prime_p(num, REPS)) { // n is prime
      if (decompose_prime(sqr, p, num)) { // if done
        mpz_set_ui(fours[0], 0);
        mpz_set_ui(fours[1], 0);
        mpz_set(fours[2], sqr);
        mpz_set(fours[3], p);
  ...
  if (mpz_tstbit(num, 0) &&  mpz_tstbit(num, 1) && mpz_tstbit(num, 2)) {
    // gmp_printf("n == 7 mod 8\n");
    // in this case: num & 7 = 7, which means num = 8m + 7 (num = 0b...111)
    // subtract largest square delta^2
    // s.t. (num > delta^2) and (delta^2 % 8 != 0)
    mpz_set(delta, sqr);
    mpz_set(num, p);
    if (!mpz_tstbit(sqr, 0) && !mpz_tstbit(sqr, 1)) { // sqr & 3 == 0
      mpz_sub_ui(delta, delta, 1); // new_delta = old_delta - 1
      mpz_add(num, num, delta); // new_n = old_n + new_delta + new_delta + 1
      mpz_add(num, num, delta);
  ...
  // post-condition: sqr = isqrt(n), num % 8 == 7, num % 4 != 0
  // and old_n = v^2 * (new_n + delta^2)
  // this implies n = a^2 + b^2 + c^2
  
  // check for special cases:
  if (special_case_p(fours, num)) {
  ...
  // Case 1: n % 4 == 3, num = x^2 + 2*p, p % 4 == 1
  //         p is a prime, p = x^2 + y^2 => n = x^2 + (y+z)^2 + (y-z)^2
  if (mpz_tstbit(num, 0) && mpz_tstbit(num, 1)) {
    // gmp_printf("CASE 1\n");
  ...
  // Case 2: n % 4 = 1 or n % 4 = 2, num = x^2 + p,
  //         p % 4 = 1, p prime, p = y^2 + z^2 implies n = x^2 + y^2 + z^2
  assert((!mpz_tstbit(num, 0) && mpz_tstbit(num, 1))
	 || (mpz_tstbit(num, 0) && !mpz_tstbit(num, 1)));
